    ...
    if new==1:
     ...
     #   print(type(alltransactions))
        ...
        if mdf.loc[(mdf['CMTE_ID'] == CMTE_ID) & (mdf['ZIP'] == ZIP) & (mdf['DATE'] == TRANSACTION_DT[4:8])].empty:
            mdf.loc[len(mdf)] = [CMTE_ID, ZIP, TRANSACTION_DT[4:8], perc, str(round(sumtransactions)), str(ntransactions)]
        else:
            mdf.loc[(mdf['CMTE_ID'] == CMTE_ID) & (mdf['ZIP'] == ZIP) & (mdf['DATE'] == TRANSACTION_DT[4:8])]=[CMTE_ID, ZIP, TRANSACTION_DT[4:8], perc, str(round(sumtransactions)), str(ntransactions)]
        print('tt',type(mdf.PERCENTILE))

        mdf['PERCENTILE'] =mdf['PERCENTILE'].astype('int')

        print(mdf['PERCENTILE'].astype('int'))
        print(mdf)
...
with open('itcont.txt') as f:
    ...
    mdf['PERCENTILE'] =str(mdf['PERCENTILE'].astype('int'))
